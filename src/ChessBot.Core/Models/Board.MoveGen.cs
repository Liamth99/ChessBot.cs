        ...
        if (startSquare.File() < 7)
        {
            if (startSquare.Rank() < 6 &&
                !_squares[startSquare + 17]
                    .IsType(_colorToMove)) // something along the lines of (this doesnt work) -> && ((_squares[startSquare + 8] | _squares[startSquare + 16]) & (piece & (Piece.Black | Piece.White)).ToggleColor()) == 0
    ...
    private void GeneratePawnMoves(byte startSquare)
    {
        ...
        if (_squares[targetByte] is Piece.None)
        {
            ...
            else
            {
                ...
                bool isStartingRank = (_colorToMove == Piece.White && startSquare.Rank() == 1) ||
                                      (_colorToMove == Piece.Black && startSquare.Rank() == 6);
    ...
    private void GeneratePawnAttacks(byte startSquare, short directionMultiplier)
    {
        ...
        if (startSquare.File() > 0)
        {
            ...
            if (leftAttackSquare is >= 0 and <= 63)
            {
                ...
                if (pieceOnLeftAttack != Piece.None && pieceOnLeftAttack.ToColor() != _colorToMove ||
        ...
        if (startSquare.File() < 7)
        {
            ...
            if (rightAttackSquare is >= 0 and <= 63)
            {
                ...
                if (pieceOnRightAttack != Piece.None && pieceOnRightAttack.ToColor() != _colorToMove ||
